...
class Compatible:
    ...
    def __init__(self, filename_v=None, filename_j=None, offset_v=50, offset_j=33, threads=None):
        dirname = os.path.dirname(__file__)
        if filename_v is None:
            self.filename_v = dirname + "/data_with_scripts/human_genes/IGHV.fasta"
        else:
            self.filename_v = filename_v
        if filename_j is None:
            self.filename_j = dirname + "/data_with_scripts/human_genes/IGHJ.fasta"
        else:
        ...
        self.rename = {"v_full": "v_call", "j_full": "j_call"}
    ...
    def group2airr(self, args):
        ...
        if len(v) == 1 and len(j) == 1:
            ...
            df["offset_v_germline_alignment"] = (
                v["germline_alignment"].values[0].replace(".", "")[: cys104 + 3]
            )
            df["v_sequence_alignment"] = df["offset_v_sequence_alignment"].str[self.offset_v :]
            df["v_germline_alignment"] = (
                df["offset_v_germline_alignment"].str[self.offset_v :].str.upper()
            )
            df["junction"] = (df.NN + df.vdj_nt).str[cys104 : cys104 + cdr3_length + 6]
            df["j_sequence_alignment"] = (
                (df.NN + df.vdj_nt).str[cys104 + cdr3_length + 3 :].str[: self.offset_j]
            )
            df["j_germline_alignment"] = (
                j["germline_alignment"].values[0][try118:].upper()[: self.offset_j]
            )
